...
/**
 * An abstract computerized edu.up.citadels.game player player. This is an abstract class, that
...
public abstract class GameComputerPlayer implements GamePlayer, Tickable {
    /**
     * the current edu.up.citadels.game state
     */
    protected Game game; // the edu.up.citadels.game object
    ...
    private boolean gameOver = false; // whether the edu.up.citadels.game is over
    private GameMainActivity myActivity; // the edu.up.citadels.game's main activity, set only
    // this edu.up.citadels.game is connected to the GUI
    private GameTimer myTimer = new GameTimer(this); // my timer

    /**
     * Returns this edu.up.citadels.game's timer.
     *
     * @return this edu.up.citadels.game's timer object
     */
     ...
     *
     * @param info
     * 			the object representing the information from the edu.up.citadels.game
     */
    ...
    private class MyRunnable implements Runnable {
        ...
        // the edu.up.citadels.game, or the timer
        public void run() {

            // if edu.up.citadels.game is over, do nothing
            if (gameOver) return;
            ...
            if (data instanceof GameInfo) { // ignore non GameInfo objects
                ...
                if (game == null) {
                    ...
                    // CASE 1: we don't know who our edu.up.citadels.game is; the only thing we're
                    ...
                    if (myInfo instanceof BindGameInfo) {
                        ...
                        game = bgs.getGame(); // set our edu.up.citadels.game
                        playerNum = bgs.getPlayerNum(); // set our player ID

                        // send a message to the edu.up.citadels.game with our player's name
