 ...
 * state of a edu.up.citadels.game. The state represented by an instance of this class can be a
 * complete state (as might be used by the main edu.up.citadels.game activity) or a partial
 * state as it would be seen from the perspective of an individual player.
 *
 * Each edu.up.citadels.game has a unique state definition, so this abstract base class has
...
public abstract class LocalGame implements Game, Tickable {
    ...
    // the stage that the edu.up.citadels.game is in
    private GameStage gameStage = GameStage.BEFORE_GAME;

    // the handler for the edu.up.citadels.game's thread
    private Handler myHandler;

    // the players in the edu.up.citadels.game, in order of  player number
    protected GamePlayer[] players;

    // whether the edu.up.citadels.game's thread is running
    ...
    private int playerReadyCount = 0; // number of players who are ready to start the edu.up.citadels.game

    // the players which have acknowledged that the edu.up.citadels.game is over
    private boolean[] playersFinished;
    private int playerFinishedCount = 0; // number of player who have so acknowledged

    // this edu.up.citadels.game's timer and timer action
    private GameTimer myTimer = new GameTimer(this);

    /**
     * Returns the edu.up.citadels.game's timer
     *
     * @return the edu.up.citadels.game's timer
     */
     ...
     *
     * @param players
     * 			the list of players who are playing in the edu.up.citadels.game
     */
    public void start(GamePlayer[] players) {
        // if the edu.up.citadels.game has already started, don't restart
        ...
        // filled during the initial message-protocol between the edu.up.citadels.game
        // and players
        this.playerNames = new String[players.length];

        // start the thread for this edu.up.citadels.game
        synchronized(this) {
        ...
        // at this point the edu.up.citadels.game is running, so set our edu.up.citadels.game stage to be that of
        // waiting for the players to tell us their names
        gameStage = GameStage.WAITING_FOR_NAMES;

        // start each player, telling them each who their edu.up.citadels.game and playerID are
     ...
     * a GameInfo object to the player. If the edu.up.citadels.game is not a perfect-information edu.up.citadels.game
     * this method should remove any information from the edu.up.citadels.game that the player is not
     * allowed to know.
     *
     * @param p
    ...
    /**
     * Notify all players that the edu.up.citadels.game's state has changed. Typically this simply
     ...
     * @return
     * 			the player's ID, or -1 if the player is not a player in this edu.up.citadels.game
     */
    ...
    /**
     * Invoked whenever the edu.up.citadels.game's thread receives a message (e.g., from a player
     * or from a timer).
     *
     * @param msg
    ...
    private void receiveMessage(Message msg) {
        if (msg.obj instanceof GameAction) { // ignore if not edu.up.citadels.game.GameAction
            GameAction action = (GameAction)msg.obj;

            // CASE 1: the edu.up.citadels.game is at the stage where we we waiting for
            ...
            if (action instanceof MyNameIsAction &&
                ...
                // edu.up.citadels.game stage, and send a message to each player that the edu.up.citadels.game is
                // about to start
                ...
                // CASE 2: we have told all players that the edu.up.citadels.game is about to start;
                ...
                // if all players are ready, set the edu.up.citadels.game stage to "during edu.up.citadels.game", and
                ...
                if (playerReadyCount >= playerNames.length) {
                    ...
                    // send each player the initial state of the edu.up.citadels.game
                    sendAllUpdatedState();
                }
            }
            else if (action instanceof TimerAction && gameStage == GameStage.DURING_GAME) {

                // CASE 3: it's during the edu.up.citadels.game, and we get a timer action
                ...
                // CASE 4: it's during the edu.up.citadels.game, and we get an action from a player
                this.checkAndHandleAction(action);
            }
            else if (action instanceof GameOverAckAction && gameStage == GameStage.GAME_OVER) {

                // CASE 5: the edu.up.citadels.game is over, and we are waiting for each player to
                // acknowledge this
    ...
    /**
     * Handles an action that is sent to the edu.up.citadels.game, checking to ensure
    ...
    private final void checkAndHandleAction(GameAction action) {
        ...
        // The move was a legal one, so presumably the state of the edu.up.citadels.game was
        // changed. Send all players the updated state.
        sendAllUpdatedState();

        // determine whether there is a winner; if so, finish up the edu.up.citadels.game
     ...
     * present point in the edu.up.citadels.game.
     *
     * @param playerIdx
    ...
    /**
     * Check if the edu.up.citadels.game is over. It is over, return a string that tells
     * who the winner(s), if any, are. If the edu.up.citadels.game is not over, return null;
     *
     * @return
     * 			a message that tells who has won the edu.up.citadels.game, or null if the
     * 			edu.up.citadels.game is not over
     */
    protected abstract String checkIfGameOver();

    /**
     * Finishes up the edu.up.citadels.game
     *
     * @param msg
     * 			The message that tells who, if anyone, won the edu.up.citadels.game
     */
    private final void finishUpGame(String msg) {

        // set the edu.up.citadels.game-stage to "over"
        ...
        // send all players a "edu.up.citadels.game over" message
        for (GamePlayer p : players) {
     ...
     *
     * @param action
     * 			The move that the player has sent to the edu.up.citadels.game
     * @return
    ...
    /**
     * sends a timer action to the edu.up.citadels.game
     */
    ...
    /**
     * Invoked whenever the edu.up.citadels.game's timer has ticked. It is expected
    ...
    // an enum-class that itemizes the edu.up.citadels.game stages
    private static enum GameStage {
        BEFORE_GAME, WAITING_FOR_NAMES, WAITING_FOR_READY, DURING_GAME, GAME_OVER
    }

    // a handler class for the edu.up.citadels.game's thread
    private static class MyHandler extends Handler {
        // the edu.up.citadels.game
        private LocalGame game;

        // constructor; parameter is expected to be this edu.up.citadels.game
