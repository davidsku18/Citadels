 ...
 *
 * is the main activity for the edu.up.citadels.game framework. To create a new edu.up.citadels.game, create a
...
public abstract class GameMainActivity extends Activity implements
    ...
    // A reference to the object representing the edu.up.citadels.game itself. This is the
    // object that knows the rules of the edu.up.citadels.game. This variable is initialized in
    // launchGame.
    private Game game = null;

    // an array containing references to all the players that are playing the edu.up.citadels.game
    private GamePlayer[] players = null;

    // tells which player, if any, is running in the GUI
    private GamePlayer guiPlayer = null;

    // whether the edu.up.citadels.game is over
    private boolean gameIsOver = false;

    // whether it is so early in the edu.up.citadels.game that the configuration screen may
    // not have been fully linked to the GUI
    private boolean justStarted = true;

    // whether the edu.up.citadels.game is in the "configuration" stage, before the actual edu.up.citadels.game
    // has started
    private boolean doingConfiguration = true;

    /**
     * contains the edu.up.citadels.game configuration this activity will be used to initialize
     */
	 ...
	 * Abstract Methods
	 *
	 * To create a edu.up.citadels.game using the edu.up.citadels.game framework you must create a subclass of
	 * GameMainActivity that implements the following methods.
	 * --------------------------------------------------------------------
	 */
    /**
     * Creates a default, edu.up.citadels.game-specific configuration for the current edu.up.citadels.game.
     *
     ...
     *         configuration for this edu.up.citadels.game. (The default may be subsequently
     ...
     * createLocalGame
     *
     * Creates a new edu.up.citadels.game that runs on the server tablet. For example, if
     ...
     *
     * @return a new, edu.up.citadels.game-specific instance of a sub-class of the LocalGame
     *         class.
     */
    public abstract LocalGame createLocalGame();

    /**
     * Creates a "proxy" edu.up.citadels.game that acts as an intermediary between a local
     * player and a edu.up.citadels.game that is somewhere else on the net.
     *
     * @param hostName
     *            the name of the machine where the edu.up.citadels.game resides. (e.g.,
     *            "upibmg.egr.up.edu")
     ...
     * onCreate
     *
     * "main" for the edu.up.citadels.game framework
     */
    @Override
    public final void onCreate(Bundle savedInstanceState) {
        ...
        // create the default configuration for this edu.up.citadels.game
        ...
        if (!this.config.restoreSavedConfig(saveFileName(), this)) {
            MessageBox.popUpMessage("Error in attempting to read edu.up.citadels.game configuration file.",
                    this);
        }

        if (this.config.isUserModifiable()) { // normal run: user has chance to modify configuration

            // initialize and show the GUI that allows the user to specify the edu.up.citadels.game's
            // configuration
            initStarterGui();
...
//		}
    ...
    /**
     * Creates the edu.up.citadels.game and players, and starts the edu.up.citadels.game.
     *
     * @param config
     *            is the configuration for this edu.up.citadels.game
     * @return
     * 			null if the launch was successful; otherwise a message telling
     * 			why edu.up.citadels.game could not be launched
     */
    private final String launchGame(GameConfig config) {

        // Set the title text with the edu.up.citadels.game's name
        this.setTitle(config.getGameName());

        // create the edu.up.citadels.game if it's local (we defer remote edu.up.citadels.game creation
        // until further down so that we do not attempt to make the
        // network connection until other errors are checked)
        if (config.isLocal()) { // local edu.up.citadels.game
            game = createLocalGame();
            // verify we have a edu.up.citadels.game
            if (game == null) {
        ...
        // create the edu.up.citadels.game if it's remote
        if (!config.isLocal()) { // remote edu.up.citadels.game
            game = createRemoteGame(config.getIpCode());
            // verify we have a edu.up.citadels.game
            if (game == null) {
                return "Could not find edu.up.citadels.game server on network.";
            }
        }
        ...
        // start the edu.up.citadels.game; then return null to indicate that the launch was
        // successful
        game.start(players);
        return null;
    ...
    protected void initTableRows() {
        ...
        // put a row in the table for each player in the edu.up.citadels.game.config
        ...
        for (int i = 0; i < numPlayers; ++i) {
            ...
            GamePlayerType[] selTypes = config.getSelTypes(); // the player types in the edu.up.citadels.game.config
            GamePlayerType[] availTypes = config.getAvailTypes(); // the available player types
    ...
    /**
     * places the data from this.edu.up.citadels.game.config into the GUI.
     *
     */
    protected void initStarterGui() {
        // do nothing without a edu.up.citadels.game edu.up.citadels.game.config
        if (this.config == null)
            return;

        // Set the title text using the edu.up.citadels.game's name
        this.setTitle(config.getGameName() + " Configuration");

        // place the pages in the tabbed dialog
        initTabs();

        // Insert a row for each player in the current edu.up.citadels.game.config
        initTableRows();
    ...
    public void onClick(View button) {
        ...
        //Save Config Button
            ...
            if (configTemp.saveConfig(saveFileName(), this)) {
                ...
                MessageBox.popUpMessage("Unable to save edu.up.citadels.game configuration.", this);
            }
        }
     ...
     * below the minimum allowed by the edu.up.citadels.game configuration.
     */
    ...
    private TableRow addPlayer() {
        ...
        //		GamePlayerType[] selTypes = edu.up.citadels.game.config.getSelTypes();
        GamePlayerType[] availTypes = config.getAvailTypes();
    ...
    public GameConfig scrapeData() {
        ...
        // First make a copy of the original edu.up.citadels.game.config without the players
        ...
        // Retrieve the info for each player and add to the edu.up.citadels.game.config
        for (TableRow row : this.tableRows) {
            ...
            //add to the edu.up.citadels.game.config
     ...
     * edu.up.citadels.game is already over).
     */
    @Override
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK && !gameIsOver) {
            // We have seen the back-key pressed, and the edu.up.citadels.game is not over;
            ...
            // otherwise (not BACK key, or edu.up.citadels.game is over), allow superclass method
            // to handle it
    ...
    /**
     * marks the edu.up.citadels.game as being over
     *
     * @param b
     * 			tells whether the edu.up.citadels.game is over
     */
