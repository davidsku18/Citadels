...
/**
 * A CitadelssPlayer object that is used as a proxy for the real player that is on another
 ...
 * unserializes the message and sends it to its edu.up.citadels.game.
 *
...
public class ProxyPlayer implements GamePlayer {
    ...
    // whether this player is ready to play the edu.up.citadels.game.  In this case, being
                ...
                new NetworkObjectPasser(null, portNum) {
                    ...
                    public void onReceiveObject(Object obj) {
                        if (obj instanceof GameAction) {
                            // if it's a edu.up.citadels.game action (which it should be), send
                            // the action to the edu.up.citadels.game
    ...
    /**
     * Tells whether the player is ready to play the edu.up.citadels.game.
     *
    ...
    /**
     * Starts the edu.up.citadels.game. (In this case the constructor has already done
     * all the work.)
     */
    public void start() {
    }
    ...
    /**
     * Used by the edu.up.citadels.game to send a GameInfo object to this player
     *
     * @param state
     * 		The state to send
     */
    public void sendInfo(GameInfo state) {
        if (game == null && state instanceof BindGameInfo) {
            ...
            // edu.up.citadels.game is), then it had better be a BindGameInfo
            // message. Get the edu.up.citadels.game from the BindGameInfo
            // object so that we have the connection for
            // future messages.
            game = ((BindGameInfo)state).getGame();
        }

        // Null out the edu.up.citadels.game from the GameInfo object (if present),
        // so that the entire edu.up.citadels.game does not get passed across the
        // network
        state.setGame(null);
    ...
    /**
     * Set this edu.up.citadels.game as a GUI. (Should never be called because the
