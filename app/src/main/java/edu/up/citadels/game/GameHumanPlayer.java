 ...
 * particular edu.up.citadels.game, a subclass should be created that can display the current
 * edu.up.citadels.game state and responds to user commands.
 *
...
public abstract class GameHumanPlayer implements GamePlayer, Tickable {
     ...
     * instance variables
     */
    protected Game game; // the edu.up.citadels.game
    ...
    private boolean gameOver; // whether the edu.up.citadels.game is over
    ...
    public GameHumanPlayer(String name) {
        ...
        // mark edu.up.citadels.game as not being over
        this.gameOver = false;
    ...
    /**
     * Returns this object's edu.up.citadels.game timer
     *
     * @return this object's edu.up.citadels.game timer.
     */
     ...
     * knows what their edu.up.citadels.game-position and opponents' names are.
     */
	 ...
	 * Abstract Methods
	 *
	 * Create the edu.up.citadels.game specific functionality for this human player by
    ...
    /**
     * Sends a 'state' object to the edu.up.citadels.game's thread.
     *
     * @param info
    ...
    private class MyRunnable implements Runnable {
        ...
        public void run() {
            ...
            // if the edu.up.citadels.game is over, just tell the activity that the edu.up.citadels.game is over
            if (gameOver) {
            ...
            if (game == null) {
                // edu.up.citadels.game has not been bound: the only thing we're looking for is
                // BindGameInfo object; ignore everything else
                if (myInfo instanceof BindGameInfo) {
                    Log.i("GameHumanPlayer", "binding edu.up.citadels.game");
                    BindGameInfo bgs = (BindGameInfo)myInfo;
                    game = bgs.getGame(); // set the edu.up.citadels.game
                    playerNum = bgs.getPlayerNum(); // set our player id

                    // respond to the edu.up.citadels.game, telling it our name
                ...
                if (myInfo instanceof StartGameInfo) {
                    Log.i("GameHumanPlayer", "notification to start edu.up.citadels.game");

                    // update our player-name array
                    allPlayerNames = ((StartGameInfo)myInfo).getPlayerNames();

                    // perform edu.up.citadels.game-specific initialization
                    initAfterReady();

                    // tell the edu.up.citadels.game we're ready to play the edu.up.citadels.game
                    game.sendAction(new ReadyAction(GameHumanPlayer.this));
                }
            }
            else if (myInfo instanceof GameOverInfo) {
                // if we're being notified the edu.up.citadels.game is over, finish up
                ...
                // acknowledge to the edu.up.citadels.game that the edu.up.citadels.game is over
                game.sendAction(new GameOverAckAction(GameHumanPlayer.this));

                // set our instance variable, to indicate the edu.up.citadels.game as over
                gameOver = true;
            }
    ...
    /**
     * callback method--called when we are notified that the edu.up.citadels.game is over
     *
     * @param msg
     * 		the "edu.up.citadels.game over" message sent by the edu.up.citadels.game
     */
    protected void gameIsOver(String msg) {
        // the default behavior is to put a pop-up for the user to see that tells
        // the edu.up.citadels.game's result
